...
def begin_chat(llm, severity=None, selected_prompt=None):
    # with st.chat_message("assistant"):
    #     st.write("How can I help you?")

    # query = st.chat_input("Tell me about your problems", key="user_input")
    # Get user input from session state instead of directly from st.chat_input
    user_input = st.session_state.get("chat_input", "selected_prompt")
    #st.write(user_input)

    if user_input:
        
        if os.path.exists(DB_FAISS_PATH):
            context_docs = get_similar_docs(user_input)
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                ...
                ("human", selected_prompt or "{input}"),
            ]
        )
        ...
        try:
            response = qa_chain.invoke({"context": context, "input": user_input, "severity": severity})
...
def main():    
    ...
    #     # Select box for example prompts
    #     selected_prompt = st.selectbox("Select an example prompt", [""] + example_prompts, key='selected_prompt')

st.markdown('<div class="fixed-bottom">', unsafe_allow_html=True)
# Display the selected prompt in the chat input
if st.session_state.selected_prompt:
    chat_input = st.text_input("Tell me about your problems", value=st.session_state.selected_prompt, key="chat_input")
else:
    chat_input = st.text_input("Tell me about your problems", key="chat_input")
